...
import com.nxt.moderagricultrue.adapter.UserAdapter;
...
//                        .getDisplayMetrics()));
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                zDataTask.get(String.format(Constants.URL_12,count,page),null,null,UserListActivity.this);
...
//                et_05.setText(null);
            ...
            case R.id.confirm:
                ...
                String s="1&name_="+prc;
                Log.e(TAG,s);
                if(prc !=null){
                    zDataTask.get(String.format(Constants.URL_12,count,s),null,null,this);
                    Log.e(TAG,String.format(Constants.URL_12,count,s));
                }
                String prs=et_02.getText().toString().trim();
                String ps="1&filed1_="+prs;
                Log.e(TAG,s);
                if(prc !=null){
                    zDataTask.get(String.format(Constants.URL_12,count,ps),null,null,this);
                    Log.e(TAG,String.format(Constants.URL_12,count,ps));
                }
...
//                break;
            ...
            public void run(){
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(UserListActivity.this, "数据已加载完毕", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        };
    ...
    private void getData(){
        if (!ComUtils.isNetWorkConnected(this)) {
            ...
            zDataTask.get(String.format(Constants.URL_12,count,page),null,null,this);
        }
    }
...
//                buyPageList.clear();
                try {
                    ...
                    buyPageList = new Gson().fromJson(s, new TypeToken<List<User>>() {
                    }.getType());
                    Log.e(TAG,buyPageList.size()+"");
                } catch (Exception e) {
                    e.printStackTrace();
                }
                mBuyPageAdapter=new UserAdapter(this, buyPageList);
                mlistview.setAdapter(mBuyPageAdapter);
            }else {
                try {
                    String s = new JSONObject(string).getString("_rows");
                    List<User>  addList = new Gson().fromJson(s, new TypeToken<List<User>>() {
                    }.getType());
                    if(addList.size()>0) {
                        buyPageList.clear();
    ...
    public void onItemClick(AdapterView<?> view, View view1, int i, long l) {
        User mBuypage=buyPageList.get(i);
        Log.e(TAG,i+"");
        startActivity(new Intent(this, UserDetailActivity.class).putExtra(Constants.VCRECNO,mBuypage));
    }

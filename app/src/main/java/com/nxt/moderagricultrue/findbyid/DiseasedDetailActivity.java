...
public class DiseasedDetailActivity extends BaseActivity {
    ...
    protected void initView() throws UnsupportedEncodingException {
        ...
        Diseased vcrecno=(Diseased) getIntent().getSerializableExtra(Constants.VCRECNO);
        if(vcrecno != null){
            tv_01.setText("地块名称："+vcrecno.getVcparceldesc());
            tv_02.setText("用药品种："+vcrecno.getVcdrug());
            tv_03.setText("用药时间："+vcrecno.getDtpharmacydate().substring(0,10));
            tv_04.setText("用药量："+vcrecno.getDtpharmacynum());
            tv_05.setText("打药方式："+vcrecno.getDtpharmacypatten());
        }
    }

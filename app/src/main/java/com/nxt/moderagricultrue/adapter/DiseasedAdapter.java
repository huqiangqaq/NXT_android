...
import com.nxt.moderagricultrue.list.DiseasedList.Update_DiseasedListActivity;
...
public class DiseasedAdapter extends ZBaseAdapter<Diseased> implements View.OnClickListener {
...
//用map来记录每个item的点击状态,默认一开始都是false
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.ll_detail:
                mContext.startActivity(new Intent(mContext, DiseasedDetailActivity.class).putExtra(Constants.VCRECNO,dataList.get(itemPosition)));
                break;
            case R.id.ll_update:
                mContext.startActivity(new Intent(mContext, Update_DiseasedListActivity.class).putExtra(Constants.VCRECNO,dataList.get(itemPosition)));
                break;
            case R.id.ll_delete:
                delete_item();
                break;
        ...
        new AlertDialog.Builder(mContext)
                ...
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        Log.d("DEL",String.format(Constants.DEL_URL_09,dataList.get(itemPosition).getVcrecno()));
                        OkHttpUtils.get().url(String.format(Constants.DEL_URL_09,dataList.get(itemPosition).getVcrecno()))
                                .build()

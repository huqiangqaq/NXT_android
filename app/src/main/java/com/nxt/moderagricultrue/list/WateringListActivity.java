...
import com.nxt.moderagricultrue.adapter.WateringAdapter;
import com.nxt.moderagricultrue.domain.Watering;
import com.nxt.moderagricultrue.findbyid.WateringDetailActivity;
...
public class WateringListActivity extends BaseActivity{
    ...
    private WateringAdapter mBuyPageAdapter;
    private List<Watering> buyPageList=new ArrayList<>();
    ...
    protected int getLayout() {
        return R.layout.activity_watering_list;
    }
...
//                        .getDisplayMetrics()));
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                zDataTask.get(String.format(Constants.URL_10,count,page),null,null,WateringListActivity.this);
...
//                et_05.setText(null);
            ...
            case R.id.confirm:
                ...
                Log.e(TAG,s);
                if(prc !=null){
                    zDataTask.get(String.format(Constants.URL_10,count,s),null,null,this);
                    Log.e(TAG,String.format(Constants.URL_10,count,s));
                }
                ...
                {
                    zDataTask.get(String.format(Constants.URL_10,count,buy),null,null,this);
                    Log.e(TAG,String.format(Constants.URL_10,count,buy));
                }
                lineShaiXuan.setVisibility(View.GONE);
                break;
            case R.id.fab:
                startActivity(new Intent(this,Add_WateringListActivity.class));
                break;
        }
    }
            ...
            public void run(){
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(WateringListActivity.this, "数据已加载完毕", Toast.LENGTH_SHORT).show();
                    }
                });
            }
        };
    ...
    private void getData(){
        if (!ComUtils.isNetWorkConnected(this)) {
            ...
            zDataTask.get(String.format(Constants.URL_10,count,page),null,null,this);
        }
    }
...
//                buyPageList.clear();
                try {
                    ...
                    buyPageList = new Gson().fromJson(s, new TypeToken<List<Watering>>() {
                    }.getType());
                    Log.e(TAG,buyPageList.size()+"");
                } catch (Exception e) {
                    e.printStackTrace();
                }
                mBuyPageAdapter=new WateringAdapter(this, buyPageList);
                mlistview.setAdapter(mBuyPageAdapter);
            }else {
                try {
                    String s = new JSONObject(string).getString("_rows");
                    List<Watering>  addList = new Gson().fromJson(s, new TypeToken<List<Watering>>() {
                    }.getType());
                    if(addList.size()>0) {
                        buyPageList.clear();

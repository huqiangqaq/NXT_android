...
public class ParcelAdapter extends ZBaseAdapter<Parcel> implements View.OnClickListener {

    private Context mContext;
    private int itemPosition = 0;
    private Map<Integer,Boolean> map = new HashMap<>();
    public ParcelAdapter(Context context, List<Parcel> dataList) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        final Holder holder;
        if (convertView == null) {
            convertView = mLayoutInflater.inflate(R.layout.item_parcel, parent, false);
            holder = new Holder();
        ...
        if (!TextUtils.isEmpty(article.getFparcelarea()+""))
            holder.tv_02.setText("地块面积（亩）："+article.getFparcelarea());
        if (!TextUtils.isEmpty(article.getVcpurpose()))
            holder.tv_03.setText("地块用途："+article.getVcpurpose());
